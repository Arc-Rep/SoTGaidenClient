...
SKILL_STAT_TYPE = {"Str", "Mag", "Skl", "Dex", "Luck"}
...
local function TestSkill(skill)
    ...
    if(skill_data["Focus"] ~= nil) then
        if(utils.contains(SKILL_FOCUS, skill_data["Focus"]) == false) then
            return false
        end
    else
        return false
    ...
    if(skill_data["Element"] ~= nil) then
        if(utils.contains(SKILL_ELEMENTS, skill_data["Element"]) == false) then
            return false
        end
    else
        return false
    ...
    if(skill_data["TargetType"] ~= nil) then
        if(utils.contains(SKILL_TARGET_TYPE, skill_data["TargetType"]) == false) then
            return false
        end
    else
        return false
    ...
    if(skill_data["FocusType"] ~= nil) then
        if(utils.contains(SKILL_FOCUS_TYPE, skill_data["FocusType"]) == false) then
            return false
        end
    else
        return false
    ...
    if(skill_data["AuraRadius"] ~= nil) then
        ...
        elseif(skill_data["AuraRadius"] ~= 0 and skill_data["AuraType"] == nil) then
            return false
        elseif(utils.contains(SKILL_AURA_TYPE, skill_data["AuraType"]) == false) then
            return false
        end
    else
        return false
    ...
    for i = 1, #skill_data["DmgIncrement"], 2 do
        if(utils.contains(SKILL_STAT_TYPE, skill_data["DmgIncrement"][i]) == false or type(skill_data["DmgIncrement"][i+1]) ~= "number") then
            return false
        end
    end
    ...
    for i = 2, #skill_data["CritMod"], 2 do
        if(utils.contains(SKILL_STAT_TYPE, skill_data["CritMod"][i]) == false or type(skill_data["CritMod"][i+1]) ~= "number") then
            return false
        end
    end
